...
// Targets the Command directly (Option B) — the pipeline can resolve it with no shim.
public sealed class CreatePersonValidator : IValidator<CreatePersonCommand>
{
    public ValidationResult Validate(CreatePersonCommand r)
    {
        ...
        if (CalculateAge(r.DateOfBirth) < 18)
            result.Add(nameof(r.DateOfBirth), "Person must be at least 18 years old.");
